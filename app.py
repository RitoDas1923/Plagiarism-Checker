...
DATA_FILE = "C:\\Users\\ritod\\OneDrive\\Desktop\\Techmela_Projects\\plagiarism-checker\\articles.csv"
NROWS = 20000
...
def create_pinecone_index():
    pinecone.create_index(name=PINECONE_INDEX_NAME, metric="cosine",dimension=300, shards=1)
...
def prepare_data(data):
    ...
    data['article_vector'] = pd.Series(list(encoded_articles))
    #Convert the article vector to a list of floats
    data['article_vector'] = data['article_vector'].swifter.apply(lambda x: list(float(i) for i in x))
    # data.to_csv('articles_with_vectors.csv')
    return data

def upload_items(data):
   # Make a list of tuples of the article id and the article vector and upload to Pinecone
    items_to_upload = []
    for i in data.iterrows():
        # Convert the article vector to a list of floats
        items_to_upload.append((str(i[1].article_id), i[1].article_vector))
    # Upload the items to Pinecone in batches of 100
    for i in range(0, len(items_to_upload), 100):
        pinecone_index.upsert(items_to_upload[i:i+100])
...
def map_titles(data):
    return dict(zip(data.article_id, data.title))

def map_publications(data):
    return dict(zip(data.article_id, data.publication))

def query_pinecone(originalContent):
    query_content = str(originalContent)
    query_vectors = model.encode(query_content)
    # Convert the query vector to a list of floats
    query_vectors = list(i.item() for i in query_vectors)
    # print(query_vectors)
    query_results = pinecone_index.query(vector=query_vectors, top_k=10)
    ...
    for items in res:
        results_list.append({
            "id": items.id,
            "title": titles_mapped[int(items.id)],
            "publication": publications_mapped[int(items.id)],
            "score": items.score,
        })
...
def search():
    if request.method == "POST":
        # print(request.form.get("originalContent", ""))
